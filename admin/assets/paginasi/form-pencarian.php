		...
		<table border="0">
			<tr>
				<td>
					<select name="Kolom" class="form-control select-column-paginasi" id="select-column-paginasi">
						...
						for ($i = 0; $i < count($column['value']); $i++) {
							?>
							<option value="<?php echo $column['value'][$i]; ?>" <?php if (isset($_GET['Kolom']) and $_GET['Kolom'] == $column['value'][$i]) {
								   echo 'selected';
							   } ?>>
								<?php echo $column['label'][$i]; ?>
							</option>
						<?php } ?>
					</select>
				</td>
				<td>
					...
					<input type="text" name="KataKunci" placeholder="Masukkan Kata Kunci..." class="form-control" value="<?php if (isset($_GET['KataKunci'])) {
						echo $_GET['KataKunci'];
					} ?>" id="input-column-paginasi">
				</td>
				<td>
